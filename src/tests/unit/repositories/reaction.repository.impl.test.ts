...
describe('getReactionByPostIdUserIdAndType', () => {
  it('should get a reaction array given a type and a post id', async () => {
    ...
    const recieved = await repository.getReactionByPostIdUserIdAndType(
  ...
  it('should get a null as the post id is not found', async () => {
    ...
    const expected = null;
    const recieved = await repository.getReactionByPostIdUserIdAndType(
      '23e0ee80-5b2b-4dd7-b31d-80a91156ae95',
      'oooo',
      'LIKE'
    );
    expect(recieved).toEqual(expected);
...
describe('getReactionByPostIdAndUserId', () => {
  it('should get a reaction array given a user id and a post id', async () => {
    ...
    const expected = [new ReactionDTO(reaction1)];
    const recieved = await repository.getReactionByPostIdAndUserId(
      '921cce9e-cfe6-4636-a0ca-9df133d38527',
      '83538af2-24e4-4435-bc36-a049183828d8'
    );
    expect(recieved).toEqual(expected);
...
describe('createReaction', () => {
  it('should create a reaction', async () => {
    ...
    const expected = new ReactionDTO(reaction1);
    const recieved = await repository.createReaction(
      new ReactionCreationDTO('921cce9e-cfe6-4636-a0ca-9df133d38527', '83538af2-24e4-4435-bc36-a049183828d8', 'LIKE')
    );
    expect(expected.id).toEqual(recieved.id);
  });
});

...
beforeEach(() => {
  ...
  user2 = {
    id: '83538af2-24e4-4435-bc36-a049183828d8',
    name: null,
...
describe('getByUsernameCursorPaginated', () => {
  it('should get all users by username, no limit and order given', async () => {
    ...
    jest
      .spyOn(mockRepository, 'getByUsernameOffsetPaginated')
      .mockResolvedValue([
        new UserViewDTO({ ...user1, follows: [], followers: [] }),
        new UserViewDTO({ ...user2, follows: [], followers: [] }),
      ]);
    const expected = [
      new UserViewDTO({ ...user1, follows: [], followers: [] }),
      new UserViewDTO({ ...user2, follows: [], followers: [] }),
...
describe('getPublicOrFollowedUser', () => {
  it('should get the user as it is being followed or has public profile', async () => {
    ...
    jest
      .spyOn(mockRepository, 'getByIdPublicOrFollowed')
      .mockResolvedValue(new UserViewDTO({ ...user1, follows: [], followers: [] }));
    const expected = new UserViewDTO({ ...user1, follows: [], followers: [] });
    const recieved = await service.getPublicOrFollowedUser(
      '83538af2-24e4-4435-bc36-a049183828d8',
  ...
  it('should throw an exception if user id is not correct', async () => {
    ...
    await expect(
      service.getPublicOrFollowedUser('83538af2-24e4-4435-bc36-a049183828d8', '3ac84483-20f1-47f3-8be1-43ab2db46ad0')
...
describe('getUserRecommendations', () => {
  it('should get all users by username, no limit and order given', async () => {
    ...
    jest
      .spyOn(mockRepository, 'getRecommendedUsersPaginated')
      .mockResolvedValue([new UserDTO(user1), new UserDTO(user2)]);
...
describe('getPreSignedURL', () => {
  ...
  it('should throw an exception if the image name is not provided', async () => {
    ...
    await expect(service.getPreSignedGetURL('', '3ac84483-20f1-47f3-8be1-43ab2db46ad0')).rejects.toThrow(
      ConflictException
    );
  });
});
...
describe('updateProfilePicture', () => {
  ...
  it('should throw an exception if the image name is not provided', async () => {
    ...
    await expect(service.updateUserProfilePicture('', '3ac84483-20f1-47f3-8be1-43ab2db46ad0')).rejects.toThrow(
      ConflictException
    );
  });
});

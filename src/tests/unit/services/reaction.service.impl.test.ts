...
describe('deleteReaction', () => {
  it('should delete a reaction given a reaction id', async () => {
    ...
    const recieved = await service.deleteReaction(
      '3cdb42b2-c12f-4bdd-bf45-1143033898fb',
  ...
  it('should throw an exception if user id is not correct', async () => {
    ...
    await expect(
      service.deleteReaction('3cdb42b2-c12f-4bdd-bf45-1143033898fb', '3ac84483-20f1-47f3-8be1-43ab2db46ad0', 'LIKE')
  ...
  it('should throw an exception if user is not the owner of the reaction', async () => {
    ...
    jest.spyOn(mockRepository, 'getReactionByPostIdUserIdAndType').mockResolvedValue(reaction1);
    await expect(service.deleteReaction('3cdb42b2-c12f-4bdd-bf45-1143033898fb', 'wrong', 'LIKE')).rejects.toThrow(
      ForbiddenException
    );
  });
...
describe('createReaction', () => {
  it('should create a reaction', async () => {
    ...
    jest.spyOn(mockRepository, 'getReactionByPostIdAndUserId').mockResolvedValue([]);
    jest.spyOn(mockRepository, 'createReaction').mockResolvedValue(reaction1);
    const expected = new ReactionDTO(reaction1);
    const recieved = await service.createReaction(
      new ReactionCreationDTO(reactionCreation.postId, reactionCreation.userId, reactionCreation.type)
    );
    expect(expected.id).toEqual(recieved.id);
  });
  ...
  it('should throw an exception if user has already reacted to this post', async () => {
    expect.assertions(1);
...
describe('getRetweetsByUserId', () => {
  ...
  it('should throw an exception if user id is not correct', async () => {
    ...
    jest.spyOn(userMockRepository, 'getById').mockResolvedValue(null);
    await expect(service.getRetweetsByUserId('83538af2-24e4-4435-bc36-a049183828d8')).rejects.toThrow(
      NotFoundException
    );
  });
